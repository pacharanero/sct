...
- A future version of `sct mcp` may add a `snomed_semantic_search` tool that uses this index.
...
```
sct embed --input <NDJSON> [--output <DIR>] [--model <MODEL>] [--batch-size <N>] [--ollama-url <URL>]
```
...
|---|---|---|
| `--input <FILE>` | *(required)* | NDJSON file produced by `sct ndjson`. Use `-` for stdin. |
| `--output <DIR>` | `snomed.lance` | Output LanceDB directory. |
| `--model <MODEL>` | `nomic-embed-text` | Ollama model name to use for embeddings. |
| `--batch-size <N>` | `64` | Number of concepts to embed per Ollama API call. |
| `--ollama-url <URL>` | `http://localhost:11434` | Ollama base URL. |
...
If Ollama is not running when you run `sct embed`, you will see:
...
This format ensures that search queries matching any of: preferred term, FSN, synonyms, or hierarchy context will find the concept.

---

## Output format

The output is a LanceDB dataset directory containing:

| Column | Type | Description |
|---|---|---|
| `id` | `string` | SCTID |
| `preferred_term` | `string` | Preferred term |
| `hierarchy` | `string` | Top-level hierarchy |
| `embedding` | `fixed_size_list<float32>[768]` | Vector embedding (768 dimensions for `nomic-embed-text`) |
...
response = ollama.embed(model="nomic-embed-text", input="heart attack")
query_vector = response["embeddings"][0]

# Search
results = table.search(query_vector).limit(10).to_pandas()
print(results[["id", "preferred_term", "hierarchy"]])
```

### From Python (manual cosine similarity)
...
# Results include _distance (lower = more similar)
results = (
...
- Embedding 831k concepts takes significant time on CPU (estimate ~30 min). A GPU or Apple Silicon machine will be much faster.
- `nomic-embed-text` produces 768-dimensional float32 vectors. Other models will produce different dimensions; the schema is determined by the first call.
- Progress is shown as concepts are embedded. If interrupted, the run can be restarted — already-written batches are preserved.
- The `sct mcp` server may gain a `snomed_semantic_search` tool in a future version that wraps this index.
