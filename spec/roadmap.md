...
- [x] `R46` **Low (stability):** MCP main loop drops invalid JSON silently (`if let Ok(msg) = serde_json::from_str`) - no JSON-RPC `-32700` reply, so a buggy client hangs awaiting a response. Already resolved before this checklist was re-checked - `11c6956` (2026-07-30) replaced the silent `if let Ok(msg) = ...` drop with an explicit match that answers unparseable JSON with the null-id `-32700` parse-error response JSON-RPC prescribes. No change needed.
