...
- [ ] `R17b-translate` **Spec-derived conformance for `ConceptMap/$translate`.** Parameters from <https://hl7.org/fhir/R4/conceptmap-operation-translate.html>; `$translate` is only wired when a `crossmaps` table exists (the fixture build provides one), so assert dispositions on a server that has it.
