...
The queue below is ordered: an agent takes the **first unstarted** entry, one operation per pull request. Each extends the shipped `R17b` spec-derived conformance pattern in [`tests/fhir_conformance.rs`](../tests/fhir_conformance.rs) to a further `sct serve` operation. For each: transcribe every R4 input parameter of the named operation from its HL7 operation page, add it to a `DISPOSITIONS` table, and give each parameter an asserted disposition - honoured (name the covering test), refused with a 4xx, or provably unable to affect the result (byte-identical to a baseline) - so a merely-ignored parameter fails the coverage test. The governing invariant is `R17`'s: unrecognised or unsupported input must never silently degrade to a broader default. Verify with `cargo test --features serve` against the committed synthetic fixture and cross-check a known concept (e.g. 22298006 = Myocardial infarction, 46635009 = Type 1 diabetes mellitus). The operations are implemented at `src/commands/serve/mod.rs` and `src/commands/serve/ops.rs`.

- [x] `R17b-lookup` **Spec-derived conformance for `CodeSystem/$lookup`.** Parameters from <https://hl7.org/fhir/R4/codesystem-operation-lookup.html>. All 7 R4 input parameters now have an asserted disposition in [`tests/fhir_conformance.rs`](../tests/fhir_conformance.rs): `code`/`property` honoured (pre-existing); `system`/`version` newly checked against the loaded release and refused on mismatch rather than silently ignored; `coding` and `date` refused (this server reads query-string parameters only, never a POST body); `displayLanguage` accepted with no effect, matching `$expand`'s single-locale handling.
- [x] `R17b-validate-code` **Spec-derived conformance for `CodeSystem/$validate-code`.** Parameters from <https://hl7.org/fhir/R4/codesystem-operation-validate-code.html>; cover both the `CodeSystem` and `ValueSet` forms the server routes (`/CodeSystem/$validate-code` and `/ValueSet/$validate-code`). All 10 `CodeSystem` and all 13 `ValueSet` R4 input parameters now have an asserted disposition in [`tests/fhir_conformance.rs`](../tests/fhir_conformance.rs): `code`/`display` honoured (pre-existing, `display` now also newly checked on the `ValueSet` form, which previously ignored it entirely); `url`/`version` (`CodeSystem`) and `system`/`systemVersion` (`ValueSet`) newly checked against the loaded release and refused on mismatch, reusing `$lookup`'s `check_lookup_system`/`check_lookup_version`; `codeSystem`/`valueSet`/`valueSetVersion`/`context`/`coding`/`codeableConcept`/`date` refused (structured POST-body datatypes, inline definitions, and historical-date validation this query-string-only single-release server cannot honour - `ValueSet`'s form of these was previously silently ignored); `abstract`/`displayLanguage` accepted with no effect (no concept here is ever FHIR-abstract; single-locale handling matches `$lookup`).
- [x] `R17b-subsumes` **Spec-derived conformance for `CodeSystem/$subsumes`.** Parameters from <https://hl7.org/fhir/R4/codesystem-operation-subsumes.html>. All 6 R4 input parameters now have an asserted disposition in [`tests/fhir_conformance.rs`](../tests/fhir_conformance.rs): `codeA`/`codeB` honoured (pre-existing); `system`/`version` newly checked against the loaded release and refused on mismatch, reusing `$lookup`'s `check_lookup_system`/`check_lookup_version` (previously silently ignored - `system` and `version` were both accepted but never read); `codingA`/`codingB` refused (structured POST-body datatypes, matching `$lookup`'s/`$validate-code`'s `coding` handling - previously silently ignored, and a request body was accepted and discarded outright, unlike the other operations).
- [x] `R17b-translate` **Spec-derived conformance for `ConceptMap/$translate`.** Parameters from <https://hl7.org/fhir/R4/conceptmap-operation-translate.html>. All 13 R4 input parameters now have an asserted disposition in [`tests/fhir_conformance.rs`](../tests/fhir_conformance.rs), against a `RefsetMode::All` fixture build so the ExtendedMap `crossmaps` table `$translate` needs is actually present: `code`/`system`/`targetsystem` honoured (pre-existing); `version` newly checked against the loaded release and refused on mismatch, reusing `$lookup`'s `check_lookup_system`/`check_lookup_version`; `url`/`conceptMap`/`conceptMapVersion` refused (this server has no notion of a distinct, versioned `ConceptMap` resource - it holds one integrated cross-terminology map for the whole loaded release); `source`/`coding`/`codeableConcept`/`dependency`/`reverse` refused (ValueSet scoping, structured POST-body datatypes, context-dependent mappings, and reverse-direction lookup this server cannot honour). Found a real defect on first read of the spec: `target` (a ValueSet-scoping URI, distinct from `targetsystem`) was silently accepted as an undocumented alias for `targetsystem` - wrong per the R4 definition - and is now refused instead of silently reinterpreted.
