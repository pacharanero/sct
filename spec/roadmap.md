...
- [ ] `R60` **Fix: `$validate-code` silently skips the `display` check when the member concept is absent from the loaded build.** Found in the 2026-08-28 bug audit (see the section below). `validate_display_for_member` (`src/commands/serve/ops.rs:1223`) returns `result=true` and **silently ignores a supplied `display`** when `fetch_concept` returns `None` - which genuinely happens for stored `.codelist` members absent from the loaded database (default builds exclude inactive concepts, and a codelist can come from a different edition/release than `--db`). The client asked "is this code in the value set *and* is this display correct" and gets an unqualified yes with the second half of the question silently dropped - the `R17` invariant violated. Remediation: when `display` is supplied and the concept cannot be fetched, keep `result=true` for membership but add a `message` part stating the display could not be verified against the loaded release (do **not** return `result=false` - membership is genuinely true and the stored set is authoritative for it); assert the new behaviour in a test alongside `valueset_validate_code_checks_display_on_both_membership_paths` in `tests/serve.rs` (build the codelist with a member id absent from the fixture DB, supply a `display`, assert `result=true` plus the unverified-display `message`). Self-contained; verify with `cargo test --features serve`.
...
- `R60` (queued above): `$validate-code` silently skips a supplied `display` when the member concept is absent from the loaded build - `validate_display_for_member`, `src/commands/serve/ops.rs:1223`.
...
6. **`sct serve` request-size/DoS bounds**: `parse_query` on a very long query string, very many repeated parameters (the `params_all` paths), and the POST-body size limit on the routes that now read a body (they refuse non-empty bodies, but is the body still fully read into memory first?).
