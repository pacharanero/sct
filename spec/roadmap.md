...
- [x] `R40` **High (stability):** the ECL recursive-descent parser (`src/ecl/parse.rs`, `parse_or -> ... -> parse_focus -> parse_sub`) had no depth limit; `sct ecl '(((((...'` with tens of thousands of parens overflowed the stack and aborted the process (reproduced: confirmed stack overflow on the pre-fix parser). Fixed: a `Parser::depth` counter with a 200-level cap now rejects over-nested parenthesised sub-expressions and attribute groups (`{ ... }`) with a clean parse error instead of recursing unbounded.
