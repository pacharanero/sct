...
- [x] `R47` **Low (security):** `--api-key` CLI flag exposes the TRUD key in `ps`/shell history on shared machines. Fixed: every use now emits a key-free stderr warning that directs users to `sct trud auth`, `--api-key-file`, or `TRUD_API_KEY`; the command help, TRUD guide, walkthrough, and command specification document the risk and safer alternatives.
