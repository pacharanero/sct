...
The `R17b` spec-derived FHIR conformance vein - `$expand`, `$lookup`, `$validate-code` (both `CodeSystem` and `ValueSet` forms), `$subsumes`, and `$translate` - is **complete and shipped** (see the changelog and git history); the `$translate` work also fixed a real silent-wrong-answer defect (`target` aliased to `targetsystem`). The items below continue the same assurance discipline: work from an authoritative source (an HL7 R4 page, or an in-repo invariant), add a test that **fails on drift**, keep the change self-contained and one-per-pull-request, and verify with `cargo test` (add `--features serve` for server items) against the committed synthetic fixture, cross-checking a known concept (e.g. 22298006 = Myocardial infarction, 46635009 = Type 1 diabetes mellitus, 73211009 = Diabetes mellitus). The governing invariant remains `R17`'s: unrecognised or unsupported input - and inaccurate self-description - must never silently mislead a client. Server operations are implemented at `src/commands/serve/mod.rs` and `src/commands/serve/ops.rs`.

- [ ] `R17c` **Spec-derived CapabilityStatement/metadata accuracy.** `/metadata` must advertise exactly the operations the server implements - no more, no less; an inaccurate CapabilityStatement is the contract lying to clients. Add a test in [`tests/fhir_conformance.rs`](../tests/fhir_conformance.rs) that: takes the operations the server actually routes in `src/commands/serve/mod.rs` (`$expand`, `CodeSystem`/`ValueSet` `$lookup`/`$validate-code`, `$subsumes`, `$translate`, and `CodeSystem` read) as the source of truth; asserts each is declared in `/metadata` against the correct resource with the correct operation name; and asserts `/metadata` does **not** advertise an operation the server does not implement. A conditionally-wired operation (`$translate`, gated on the crossmaps table via `translate_available`) must be advertised **iff** it is actually available - verify both states (a `Simple`-mode build and a `RefsetMode::All` build, mirroring the `$translate` conformance test). Fixture-verifiable; no new dependency.
- [ ] `R17d` **`OperationOutcome` shape conformance for error paths.** Every error response from `sct serve` must be a well-formed FHIR `OperationOutcome` with the correct HTTP status and `issue.severity`/`issue.code`. The `R17b` defects returned well-formed *wrong* resources; a **malformed** `OperationOutcome` is the complementary failure mode this catches. Transcribe the intended mapping from `FhirError` (in `src/commands/serve/`) into a table - e.g. not-found -> 404 + `issue.code=not-found`, invalid input -> 400 + `issue.code=invalid`, unsupported/refused input -> 400, request timeout -> its chosen status - and add a test that drives each error path over HTTP and asserts the status, `resourceType == "OperationOutcome"`, and the issue `severity`/`code`. Fixture-verifiable; no new dependency.
- [ ] `R57` **Codelist <-> ECL round-trip property test.** `sct codelist export --format ecl` must round-trip with `sct codelist add --ecl`: exporting a codelist's active members to ECL and re-importing that ECL into a fresh codelist must yield the same active membership. Add a round-trip test that builds a codelist against the committed fixture (e.g. the descendants of `<<73211009` = Diabetes mellitus), exports to ECL, re-imports into a new codelist, and asserts the two active member sets are identical. Cross-check a known member (46635009 = Type 1 diabetes mellitus). Self-contained; surfaces are `sct codelist export`/`add` (see [`commands/codelist`](../docs/commands/codelist.md) and the codelist module). Verify with `cargo test` against the fixture.
