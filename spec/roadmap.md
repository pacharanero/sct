...
- [ ] `R39` **High (stability):** `src/index/query.rs` `preferred_term()` slices the mmap'd terms index unchecked: `count` comes from the file's first 4 bytes, entry slices `entries[mid*16..]` and the terms-text slice `self.mmap[start+off..start+off+len]` all trust file-supplied values. A truncated/corrupt `.fst` index panics any search that hits. The neighbouring `read_postings` is bounds-checked - mirror it: validate `count <= entries.len()/16` and use `.get(..)` returning `None`. Verified.
- [ ] `R40` **High (stability):** the ECL recursive-descent parser (`src/ecl/parse.rs`, `parse_or -> ... -> parse_focus -> parse_sub`) has no depth limit; `sct ecl '(((((...'` with tens of thousands of parens overflows the stack and aborts - and the same expression via MCP or FHIR `$expand` kills the long-running server. Add a depth counter (~200 cap) returning a parse error. Verified.
- [ ] `R41` **Medium (security):** TRUD API key can leak into error output: URLs embed the key (`.../keys/{api_key}/items/...`, `src/commands/trud.rs`) and the fallthrough error arms print ureq transport errors whose Display includes the full URI (DNS/TLS/redirect failures). Keys land on stderr/CI logs. Redact the key before formatting or map transport errors to a generic message. Verified.
- [ ] `R42` **Medium (stability):** MCP Content-Length framing (`src/commands/mcp.rs`): (a) malformed/overflowing lengths silently map to 0, and `len == 0` returns `Ok(None)` which the main loop treats as EOF - one bad header cleanly terminates the whole MCP session with no diagnostic; (b) `vec![0u8; len]` allocates whatever the header claims - a rogue local client can demand GiB. Return an error for unparseable lengths and cap accepted lengths (~16 MiB). Verified.
- [ ] `R43` **Medium (stability):** `src/commands/read2.rs` decompresses a zip entry fully into memory (`read_to_end`) with no size cap - a zip bomb passed as the DMWB/Read2 archive OOMs. Check `entry.size()` against a cap and read via `.take(cap)`. *(agent-reported)*
- [ ] `R44` **Low (security):** `sct serve` with `--host 0.0.0.0` exposes an unauthenticated API; `$expand` materialises full ECL result sets per request (FTS capped, ECL uncapped), so remote clients can drive memory/CPU. Availability risk only (public terminology data). Warn prominently on non-loopback bind; consider capping/streaming ECL expansion. *(agent-reported)*
- [ ] `R45` **Low (stability):** TRUD downloads rename `*.tmp` into place without `sync_all()` - crash straight after can leave a complete-looking but truncated file; SHA-256 was computed on the stream, not re-verified from disk. Fsync before rename. *(agent-reported)*
- [ ] `R46` **Low (stability):** MCP main loop drops invalid JSON silently (`if let Ok(msg) = serde_json::from_str`) - no JSON-RPC `-32700` reply, so a buggy client hangs awaiting a response. *(agent-reported)*
