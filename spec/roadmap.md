...
**Empty.** `R12`'s two remaining compression pieces (straddling-exclusion push-down and `^refset` cover clauses, specified with worked examples in [`commands/ecl-compress.md`](commands/ecl-compress.md)) are both shipped - see the changelog and git history. The only other candidate, `R16` - the practical FHIR surface, **one piece per pull request** (all 21 R4 `$expand` parameters now have an asserted disposition per `tests/fhir_conformance.rs`, `CodeSystem` resource read - `GET /CodeSystem` and `GET /CodeSystem/{id}` - has shipped, and stored ValueSets now support a front-matter `canonical_url` override plus `GET /ValueSet?status=` draft filtering) - is not currently eligible either. What remains there is the useful FHIR R5 additions, but no specific first R5 operation/parameter is scoped yet - a human should pick one before this returns to the autonomous queue. Note that `$expand` has no `property` parameter in R4 - that is an R5 addition, so it belongs with this remaining work rather than the R4 surface above.
