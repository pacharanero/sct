...
- [x] `R17c` **Spec-derived CapabilityStatement/metadata accuracy.** Shipped: `/metadata`'s CapabilityStatement is checked against the operations `build_router` actually routes (`$expand`, `CodeSystem`/`ValueSet` `$lookup`/`$validate-code`, `$subsumes`, `$translate`, and `CodeSystem` read/search-type), including the conditional `$translate` advertisement, in `tests/fhir_conformance.rs` (`metadata_declares_exactly_the_routed_operations_simple_mode`/`_all_mode`, `metadata_omits_translate_when_no_crossmaps_table`).
- [x] `R17d` **`OperationOutcome` shape conformance for error paths.** Shipped: `tests/fhir_conformance.rs` transcribes the `FhirError` -> HTTP mapping (`not_found`/404, `invalid`/400, the `reject_xml` `not-supported`/406, `exception`/500, `too_costly`/403, `timeout`/408) into an `ERROR_MAPPINGS` table and asserts each produces a well-formed single-issue `OperationOutcome` with the right status and `issue.severity`/`issue.code`. The four HTTP-reachable paths (`not_found`, `invalid`, `not-supported`, and `exception` - triggered by dropping `concept_isa`, mirroring `R17c`'s `crossmaps`-drop technique) are driven over live HTTP; `too_costly` and `timeout` are checked directly against `FhirError::outcome()` because reaching them over HTTP needs either >100k fixture rows or waiting out the real 30-second `REQUEST_TIMEOUT` - both are already proven reachable through the production code path by the existing `ops`-level tests in `tests/serve.rs`.
- [x] `R57` **Codelist <-> ECL round-trip property test.** Shipped: `tests/cli.rs`'s `codelist_export_ecl_round_trips_through_add_ecl` builds a codelist against the committed synthetic fixture via `sct codelist add --ecl` (everything under Diabetes mellitus except Type 1, `<<73211009 MINUS <<46635009`), exports it back with `sct codelist export --format ecl`, re-imports that expression into a fresh codelist via `sct codelist add --ecl`, and asserts the two active member sets are identical by checking presence/absence of all three concepts in the fixture's Diabetes mellitus subtree (73211009 = Diabetes mellitus, 44054006 = Type 2 diabetes mellitus, 46635009 = Type 1 diabetes mellitus). Runs over the real `sct sqlite`-built database and the full CLI surface, distinct from the in-memory `compress`-level unit tests in `src/commands/codelist.rs` (`export_ecl_round_trips_a_clean_subtree` etc.) that check the same claim at the function level.
