...
- [ ] `R1` **Ship the Rust SDK facade and make existing surfaces consume it.** Add an idiomatic `sct_rs::sdk::Snomed` API for read-only open, provenance, typed concept lookup/search, ECL expansion, hierarchy/subsumption, refsets, mappings/history, codelists, and optional FST autocomplete; return typed serialisable records and typed errors, never print from SDK methods, and migrate CLI/MCP/FHIR query logic incrementally so there remains one engine. Keep `default-features = false` lean, publish compiling Rustdoc/downstream examples, document the pre-1.0 compatibility policy, and add top-level **SDK** docs navigation with Overview, Rust, and Data and licensing pages.
