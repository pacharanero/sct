...
The `R17b` spec-derived FHIR conformance vein (`$expand`, `$lookup`, `$validate-code` in both `CodeSystem` and `ValueSet` forms, `$subsumes`, `$translate`), the `R17c` CapabilityStatement-accuracy and `R17d` `OperationOutcome`-shape checks, and the `R57` codelist<->ECL round-trip are all **complete and shipped** (see the changelog and git history); `$translate` also fixed a real silent-wrong-answer defect (`target` aliased to `targetsystem`), and `R17c` caught the CapabilityStatement under-declaring `CodeSystem` read/search-type. The items below continue the same assurance discipline: work from an authoritative source (an HL7 R4 page, or an in-repo invariant), add a test that **fails on drift**, keep the change self-contained and one-per-pull-request, and verify with `cargo test` (add `--features serve` for server items, `--test cli` for CLI items) against the committed synthetic fixture, cross-checking a known concept (e.g. 22298006 = Myocardial infarction, 46635009 = Type 1 diabetes mellitus, 73211009 = Diabetes mellitus). The governing invariant remains `R17`'s: unrecognised or unsupported input - and inaccurate self-description - must never silently mislead a client. Server operations are implemented at `src/commands/serve/mod.rs` and `src/commands/serve/ops.rs`.

- [ ] `R58` **MCP output-schema conformance for every data-returning tool.** Issue [#106](https://github.com/pacharanero/sct/issues/106) shipped a schema that every list-tool row violated (`active` missing under `additionalProperties: false`) because nothing validated a real tool response against its declared `outputSchema`. The fix added coverage for `snomed_search`/`snomed_children`/`snomed_ancestors`/`snomed_concept`/`snomed_map` plus a struct-serialisation check, but the refset and codelist tools are still unverified over the live path (the in-memory `build_test_db` has no reference sets). Extend the coverage so **every** data-returning MCP tool has its real `structuredContent.data` validated against its own `output_schema` with the existing `assert_conforms` recursive checker (`additionalProperties`/`required`, recursing through `properties`/`items`) in `src/commands/mcp.rs`. Cover at least `snomed_refsets`, `snomed_refset_members`, `snomed_refset_compare`, `snomed_refset_profile`, and the codelist tools (`codelist_list`/`codelist_show`/`codelist_stats`/`codelist_validate`), giving each the data it needs - extend `build_test_db` with reference-set/crossmap rows, or drive the test from a real `sct sqlite` fixture build (as `tests/fhir_conformance.rs::build_db` does, which satisfies the verification contract and already has refsets). Assert each covered tool returns non-empty data so the check is never vacuous; skip only `snomed_semantic_search` (needs an embeddings artefact), with a recorded reason. Reuses `call_tool_for_test` and `assert_conforms`. Verify with `cargo test`, cross-checking a known concept/refset.
