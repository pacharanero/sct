...
2. **`sct diff` and `sct history`**: association-forwarding edge cases (a concept retired and re-activated across releases; forwarding chains that loop). No conformance tests exist for loop termination.
...
4. **FST index vs SQLite drift** (`sct fst` built from an older NDJSON than `--db`): is there a provenance check refusing a mismatched FST, or does autocomplete silently serve stale terms? (`sct semantic` got this right via Arrow-metadata profile checks - verify the FST path has an equivalent.)
