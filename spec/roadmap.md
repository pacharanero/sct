...
- [ ] `R17b-validate-code` **Spec-derived conformance for `CodeSystem/$validate-code`.** Parameters from <https://hl7.org/fhir/R4/codesystem-operation-validate-code.html>; cover both the `CodeSystem` and `ValueSet` forms the server routes (`/CodeSystem/$validate-code` and `/ValueSet/$validate-code`).
