...
1. **ECL parser robustness at the serve boundary**: malformed/hostile ECL via `?fhir_vs=ecl/...` and MCP - look for stack depth, unbounded recursion, or panics in `src/ecl/parse.rs` reachable from user input (the parser has bounded-chain tests; check the *unbounded* shapes: deep nesting, long attribute chains, pathological whitespace).
