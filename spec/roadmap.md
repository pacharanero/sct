...
1. **TRUD download resume/partial-file handling** (`src/commands/trud.rs`): what happens on a truncated zip after a mid-download failure - is the `.part`/fsync path atomic against a crash, and is a stale partial ever picked up as complete?
2. **Python bindings error mapping** (`python/`): do Rust errors cross the FFI boundary as typed exceptions or as a generic `RuntimeError` with the message flattened - and can a panic cross at all (abort vs exception)?
3. **Windows path handling in codelist roots** (`CodelistRoot`): the traversal/symlink tests are Unix-gated; verify the Windows equivalents (junctions, `..` with backslashes, drive-relative paths) - `resolve_new_file` is the entry point.
...
- [ ] `R64` **LOINC download tooling, TRUD-parallel.** LOINC publishes a download API ([loinc.org/kb/api/download](https://loinc.org/kb/api/download)); investigate its authentication model (downloads require a free LOINC account) and, in the same shape as `sct trud`: implement `sct loinc download` (and a `check` for newer releases) with credential handling via key file/env/config - never argv - checksum verification, fsync-before-persist, and the LOINC version captured into provenance. Sequenced after `R61`'s ingest lands (download tooling is only worth building once the artefact it feeds exists) and human-gated on the API investigation plus a LOINC account.
