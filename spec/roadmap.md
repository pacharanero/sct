...
**The queue is currently empty** - no `[ ]` item is listed above. Per the closing instruction below, say so rather than substituting unlisted work; the one exception is the pre-sanctioned bug-audit leads in the next section, which an agent may still pick up.
