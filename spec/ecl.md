...
Concept references may carry an optional `\|term\|` label: `73211009 |Diabetes mellitus|`. It plays no part in set evaluation (the id alone decides membership), but `R69` (shipped) checks it against the concept's actual descriptions - the FSN with and without its semantic tag, the preferred term, and every synonym, case-insensitively - when the database is available. A mismatch prints a stderr warning naming the supplied term and the concept's actual term(s), so a human reading the expression is not reassured by a label that is silently wrong; if the supplied term happens to be another concept's actual term, that concept is named too, since a transposed identifier is the likeliest way to arrive here. This is a warning, not a validation failure - a concept's preferred term legitimately changes between releases, and a saved codelist or ECL expression must not break on upgrade - so the expression still evaluates normally either way. Implementation: the term is retained through lexing (`src/ecl/lex.rs`) into `Expr::Concept`'s second field (`src/ecl/ast.rs`) and checked in `src/ecl/terms.rs` at evaluation time (`src/ecl/eval.rs`), so every caller of `evaluate`/`evaluate_with_tct` gets it - `sct ecl expand`, `sct codelist add --ecl`, and `sct serve` `$expand`'s general path. The FHIR server's SQL fast path for a bare hierarchy/refset expression (`simple_op` in `src/commands/serve/ops.rs`) bypasses the full evaluator for performance and therefore skips this check; a mismatched annotation there simply evaluates silently, same as before `R69`.
